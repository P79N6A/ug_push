  ...
  <div v-if="data">
    <el-dialog title="编辑角色用户" :visible.sync="dialogVisible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" icon="el-icon-edit" title="编辑" size="small" @click="setEvent()">
          <span class="hidden-xs-only">保存</span>
        </el-button>
      </div>
    </el-dialog>
    <el-button v-if="data.role_type === 0" type="primary" icon="el-icon-edit" title="编辑" size="mini" @click="dialogVisible = true">
      <span class="hidden-xs-only">编辑</span>
    </el-button>
    <el-button v-else-if="data.role_type === 1" type="primary" title="查看详情" size="mini">
      <a href="/hot/#/category/center/accredit" target="_blank">查看详情</a>
    </el-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    setEvent() {
      const data = {
        url: this.apiconf.setRoleAccount,
        params: {
