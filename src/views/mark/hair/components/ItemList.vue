    ...
    <el-col :span="24" class="item-wrapper clearfix">
      <h3 style="padding-left: 20px;height:30px;line-height:30px;">
        ...
        <el-button
          ...
          plain
        >已下线</el-button>
        <span v-if="item.check_user" style="font-weight: 400">审核人：{{item.check_user}}</span>
      </h3>
      <div class="item-select">
        <span style="white-space: nowrap;">发色分类：
          <span style="color:green">{{activeState.color}}</span>
        </span>
        <el-select style="margin-left:10px" v-model="item.color_state" placeholder="请选择">
          <el-option
        ...
        <span style="margin-left:20px;white-space: nowrap;">长短分类：
          <span style="color:green">{{activeState.len}}</span>
        </span>
        <el-select style="margin-left:10px" v-model="item.len_state" placeholder="请选择">
          <el-option
            v-for="item in lenOptions"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          ></el-option>
        </el-select>
        <span style="margin-left:20px;white-space: nowrap;">遮挡分类：
          <span style="color:green">{{activeState.cover}}</span>
        </span>
        <el-select style="margin-left:10px" v-model="item.cover_state" placeholder="请选择">
          <el-option
      ...
      <div class="draw-img">
        <div class="draw-img-content">
          <div class="btn-wrap">
            ...
            <el-button type="primary" id="blackRoundBtn">黑圈</el-button>
            <el-button type="primary" id="whiteRoundBtn">白圈</el-button>
            <el-button type="primary" id="autoBtn">线条</el-button>
            ...
            <el-slider
              v-if="painterWidthState"
              v-model="painterWidth"
              class="range"
              show-input
              :min="1"
              :max="50"
          ...
          <div v-show="roundState" id="round" class="round"></div>
...
import $ from "jquery";
import { hairAdmin } from "../../authlist";
...
export default {
  ...
  data() {
    return {
      ...
      canvasOpacity: 0.5,
      imgMaskUrl: "",
      imgUrlHeader: "http://10.85.125.61:13145",
      loading: "",
      halfWidth: 12.5,
      isFirst: true,
      activeState: {
  ...
  methods: {
    changeActiveState() {
      if (this.item.color_state == 1) {
        this.activeState.color = "黑色";
      } else if (this.item.color_state == 2) {
        this.activeState.color = "其他";
      }
      if (this.item.len_state == 1) {
        this.activeState.len = "长";
      } else if (this.item.len_state == 2) {
        this.activeState.len = "中";
      } else if (this.item.len_state == 3) {
        this.activeState.len = "短";
      }
      if (this.item.cover_state == 1) {
        this.activeState.cover = "是";
      } else if (this.item.cover_state == 2) {
        this.activeState.cover = "否";
      }
    },
    imgStateCheck(state) {
      var tempData = {
        url: "http://10.85.125.61:13145/check_data.json", // 接口请求地址
        param: {
          check_type: state,
          id: this.item.id,
          color_state: this.item.color_state,
          len_state: this.item.len_state,
          cover_state: this.item.cover_state
        ...
        res => {
          if (res && res.code == 200) {
            if (state == 1 || state == 2) {
              ...
              this.item.isLabel = 2;
              this.item.check_user = this.eamil;
            } else if (state == 4) {
              Message({
                showClose: true,
                message: res.msg || "取消下线成功！",
                type: "success",
                duration: 1500
              });
              this.item.isLabel = 1;
              this.item.check_user = "";
            }
          } else {
            if (state != 3) {
              Message({
    ...
    saveImg() {
      ...
      saveImg.onload = () => {
        ...
        const tempData = {
          url: "http://10.85.125.61:13145/push_result.json", // 接口请求地址
          param: {
            id: this.item.id,
            pic_data: url,
    ...
    imgLoad() {
      ...
      $("#blackRoundBtn").click(() => {
        $(".canvas-cover")
          .off("mousedown")
          .on("mousedown", this.drawRound("black"));
      });
      $("#whiteRoundBtn").click(() => {
        $(".canvas-cover")
          .off("mousedown")
          .on("mousedown", this.drawRound("white"));
      });
      $("#autoBtn").click(() => {
        $(".canvas-cover")
          .off("mousedown")
          .on("mousedown", this.drawAuto("black"));
      });
      $("#recallBtn").click(() => {
    ...
    drawAuto(color) {
      ...
      function endAction() {
        //移除鼠标移动事件、
        endSite.x = event.pageX - $("#box").offset().left;
        endSite.y = event.pageY - $("#box").offset().top;
        if (endSite.x > statrSite.x) {
          context.lineTo(maxSite.x, maxSite.y);
          context.lineTo(canvasEle.width, maxSite.y);
          context.lineTo(canvasEle.width, canvasEle.height);
          context.lineTo(0, canvasEle.height);
          context.lineTo(0, minSite.y);
          context.lineTo(minSite.x, minSite.y);
        } else {
          context.lineTo(minSite.x, minSite.y);
          context.lineTo(0, minSite.y);
          context.lineTo(0, canvasEle.height);
        ...
        context.fill();
        that.isClear = false;
        $(".canvas-cover").off("mousemove");
        $(".canvas-cover").off("mouseup");
        $(".canvas-cover").off("mouseleave");
      }
      function moveAction(event) {
        ...
        if (mouseX1 >= maxSite.x) {
          maxSite.x = mouseX1;
          maxSite.y = mouseY1;
        }
        if (mouseX1 <= minSite.x) {
          minSite.x = mouseX1;
          minSite.y = mouseY1;
        }
        context.lineTo(mouseX1, mouseY1);
    ...
    drawRound(color) {
      ...
      function endAction() {
        ...
        context.fill();
        that.isClear = false;
        $(".canvas-cover").off("mousemove", moveAction);
        $(".canvas-cover").off("mouseup");
      }
      function moveAction(event) {
        let mouseX1 = event.pageX - $("#box").offset().left;
    ...
    clearEventListen() {
      $(".canvas-cover").unbind();
      $(".draw-img-content").unbind();
      $(".draw-img-content").unbind();
    },
    drawLine() {
      const that = this;
      this.roundState = true;
      this.painterWidthState = true;
      this.clearEventListen();
      //添加鼠标按下抬起事件
      let canvasEle = $("#box")[0];
      let context = canvasEle.getContext("2d");
