<template>
    <el-main v-if="pageAuth === 1">
        <el-row class="pane-title-wrap">
            <h6 class="pane-title">mid协同正向页面</h6>
        </el-row>
        <el-row class="search-form-wrap">
            ...
            <div v-if="dataList && dataList.length > 0">
                <item-list
                :key="mid.sim_info.sim_mid+idx"
                :index="idx+1"
                :sim_info="mid.sim_info"
...
export default {
    ...
    methods: {
        init() {
            if (this.pageAuth === 1 && this.$route.query.mid) {
                this.onSearch();
            }
        },
        onSearch() {
            ...
            const tempData = {
                url: 'http://10.77.29.69:8800/show_mid_cf_info',
            ...
            this.$store.dispatch('ProxyGet', params).then(res => {
                if (res.code && Number(res.code) == 200) {
                    ...
                    this.dataList = res.sim_results;
                } else {
                    Message({
                        showClose: true,
                        message: "mid协同正向页面数据请求失败",
                        type: "error",
                        duration: 1000
                    })
                }
