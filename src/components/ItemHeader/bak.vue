<template>
  <div>
    <el-header height="auto">
      ...
      <el-row type="flex" justify="center" class="nav-wrapper" v-if="isLogin">
        <!-- <el-menu
        ...
        background-color="#304156"
        text-color="#fff"
        active-text-color="#ffd04b" v-if="menulist">
          ...
          <el-submenu :key="obj.id" :index="`${obj.id}`" v-for="obj in menulist">
            ...
            <item-menu :key="item.id" :preindex="`${obj.id}`" :item="item" v-for="item in obj.list" v-if="obj.list"></item-menu>
          </el-submenu>
        </el-menu>
      </el-row>
      <el-main v-else-if="isLogin == null">Loading···</el-main>
      <el-main v-else>请先登录</el-main>
      <el-main v-if="auth == 'response failed'">
        <el-alert
        title="网络异常，请重试一下"
        type="info"
        show-icon
...
export default {
  ...
  computed: {
    ...mapGetters(["email", "isLogin", "auth", "menulist"])
  },
  methods: {
    init() {
