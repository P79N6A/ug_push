  ...
  <div v-if="data">
    <el-dialog :title="`【${data.username}】用户 频道授权`" :visible.sync="dialogVisible">
      <el-form>
        ...
        <el-form-item label="授权频道" label-width="120px" required v-if="allChannel">
...
export default {
  ...
  computed: {
    ...mapGetters(["email", "allChannel"])
  },
  props: ["data", "callback", "defaultChannel"],
  methods: {
    init() {
    ...
    deleteEvent() {
      ...
      this.$store.dispatch("DeleteUserChannels", data).then(res => {
    ...
    updateEvent() {
      ...
      this.$store.dispatch("SetUserChannels", data).then(res => {
        if (res.code && Number(res.code) == 200) {
          this.callback();
        } else {
          Message({
