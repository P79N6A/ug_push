  ...
  <el-row>
    <el-col :xs="24" :sm="23" :md="23" :lg="23" :xl="23" class="item-wrapper">
      <el-row>
        <div class="item-status-box clearfix">
          ...
          <div class="clearfix left">
            ...
            <label class="item-state left">物料状态: {{getState(item.materials_type)}}</label>
          </div>
          <div class="clearfix right">
            <el-button v-if="item.materials_type != '-1'" type="danger" plain size="mini" class="right" style="margin-left: 10px;" @click="deleteEvent">删除</el-button>
            <el-button v-if="item.materials_type != '1'" type="success" plain size="mini" class="right" @click="passEvent">通过</el-button>
        ...
        <div style="margin: 10px;">
          <el-form :inline="true" label-position="left" label-width="50px">
            <el-form-item label="分类1" size="mini" v-if="categoryList">
              <el-select v-model="cate" multiple placeholder="请选择" filterable :disabled="!isEditable">
                <el-option v-for="obj in categoryList" :key="obj.tagid" :label="obj.name" :value="obj.tagid">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="分类2" size="mini" v-if="secondTagList">
              <el-select v-model="second_tag" multiple placeholder="请选择" filterable :disabled="!isEditable">
                <el-option v-for="obj in secondTagList" :key="obj.tagid" :label="obj.name" :value="obj.tagid">
                </el-option>
              </el-select>
            </el-form-item>
        ...
        <!-- <div style="padding-bottom: 10px;" v-if="logList">
      ...
      <el-row class="img-wrapper" v-if="item.pic_ids && item.pic_ids.length > 0">
        <el-col style="margin: 2px" :xs="7" :sm="7" :md="5" :lg="4" :xl="20" :key="idx" v-for="(val, idx) in item.pic_ids">
          <div class="img-box">
            <div class="bg-img" :style="`background-image: url(//ww2.sinaimg.cn/bmiddle/${val}.jpg);`">{{!val && '图片加载失败' || ''}}</div>
          </div>
        </el-col>
      </el-row>
      <el-row class="video-wrapper" v-if="item.video_pic_urls && item.video_pic_urls.length > 0">
  ...
  methods: {
    ...
    remoteMethod(keyword) {
      if (keyword !== "") {
        this.loading = true;
        const tempData = {
          url: "http://10.85.115.126:6066/query_tag",
          param: {
            tag_name: keyword
          }
        };
    ...
    saveEvent() {
      this.isEditable = false;
      const tempData = {
        url: "http://10.85.115.126:6069/update_mid",
        param: {
          mids: this.item.mid,
