<template>
  <el-container direction="vertical">
      ...
      <el-row type="flex" justify="center" class="table-wrap" v-if="list">
        <el-col :xs="24" :sm="23" :md="23" :lg="23" :xl="23">
          ...
          <el-table :data="list" style="width: 100%" max-height="400" stripe :summary-method="getSummaries" show-summary sum-text="总计">
            ...
            <el-table-column align="center" width="100" fixed prop="age" label="年龄" :filters="[{text: '70后', value: '70'},{text: '80后', value: '80'},{text: '90后', value: '90'},{text: '00后', value: '00'},{text: '其他', value: 'other'},{text: '全部', value: 'all'}]" :filter-method="filterHandler">
              <template slot-scope="scope">
                <el-tag :type="scope.row.age === 'all' ? 'danger' : 'success'">{{ setAge(scope.row.age) }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column align="center" width="100" fixed prop="sex" label="性别" :filters="[{text: '男性', value: 'male'},{text: '女性', value: 'female'},{text: '其他', value: 'other'},{text: '全部', value: 'all'}]" :filter-method="filterHandler">
              <template slot-scope="scope">
    ...
    <el-main v-else-if="usergroup != null && usergroup != 'response failed'" class="el-noauth">
      <auth></auth>
    </el-main>
    <div class="maskerSkip" v-show="showMasker"></div>
    <div class="maskerTips" v-show="showMasker">
      页面内容已迁移至
      <a href="/hot/#/hotcenter/analysis/material?type=3" title="跳转至【热门管理-物料分析-单条物料分析】">热门管理-物料分析-单条物料分析</a>当前页面即将废弃~</div>
  </el-container>
</template>
