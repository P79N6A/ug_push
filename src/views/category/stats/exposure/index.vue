<template>
  <el-main v-if="pageAuth === 1">
    <el-row class="pane-title-wrap">
      <el-col :span="24">
        <h6 class="pane-title">
          <i class="el-icon-document hidden-xs-only" style="margin-right: 6px;"></i>UID曝光查询</h6>
      </el-col>
    </el-row>
      ...
      <el-col :span="24" v-if="cateList && cateList.length > 0">
        <el-form inline :model="formInline" size="mini">
          <el-form-item label="UID">
            <el-input v-model="formInline.uid" placeholder="请输入UID" clearable></el-input>
          </el-form-item>
          <el-form-item label="领域" v-if="!hasAllAuth">
    ...
    <el-row class="content-box-wrap">
      <el-col :span="24" v-if="reportData">
        <el-table :data="reportData" style="width: 100%" max-height="500" border stripe size="mini">
          <el-table-column width="100" prop="exposure_date" label="日期" fixed>
          </el-table-column>
          <el-table-column width="130" prop="uid" label="uid">
            <template slot-scope="scope">
              <user-detail :uid="curUid" :iswhite="1"></user-detail>
            </template>
          </el-table-column>
          <el-table-column prop="" label="昵称">
            <template slot-scope="scope">
              <a :class="scope.row.user_name ? 'link-btn' : 'errorBtn'" :href="`//weibo.com/u/${curUid}`" target="_blank" title="跳转至微博主页">{{scope.row.user_name || '未知'}}</a>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="count_send_mid" label="当天发布当天曝光条数" width="170">
            <template slot-scope="scope">{{_formatNum(scope.row.count_send_mid)}}</template>
          </el-table-column>
          ...
          <el-table-column align="left" label="操作项">
            <template slot-scope="scope">
              <operate :data="scope.row" :uid="formInline.uid"></operate>
            </template>
          </el-table-column>
        </el-table>
      </el-col>
...
export default {
  ...
  data() {
    return {
      apiConf: {
        ...
        getExposureListByUid: "/field/getExposureListByUid"
      },
      formInline: {
        uid: "",
        start_time: "",
  ...
  methods: {
    ...
    onSearch() {
      ...
      const data = {
        url: this.apiConf.getExposureListByUid,
        params: {
          ...this.formInline,
          username: this.email,
          cip: 230
