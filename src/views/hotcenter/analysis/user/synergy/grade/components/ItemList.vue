    ...
    <el-col :xs="24" :sm="23" :md="23" :lg="23" :xl="23" class="item-wrapper">
      <el-row>
        <div class="item-status-box clearfix">
          <span>No.{{index}}</span>
          <a class="item-status-mid" :href="item ? item.link : `https://m.weibo.cn/status/${infomid}`" target="_blank" title="查看微博详情">{{infomid}}</a>
          <p class="item-mid-info">评估打分：{{infosim}}</p>
        </div>
      </el-row>
      <show-batch :item="item"></show-batch>
      <el-row type="flex" class="item-footer" v-if="item">
...
export default {
  ...
  props: ["item", "index", "infosim", "infomid"],
  data() {
    return {
      formatNum
    };
