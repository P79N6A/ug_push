<template>
  <el-aside v-if="menulist && menuArr" :width="isPhone && !showmenu ? '0px' : '250px'" :class="isPhone ? 'app-wrapper-aside' : ''">
    <el-row class="nav-wrapper">
      ...
      <el-menu :router="true" :default-active="$route.path" class="el-header-menu" mode="vertical" background-color="#304156" text-color="#fff" active-text-color="#ffd04b">
        <item-menu :key="val" :icon="key" :item="menulist[val]" v-for="(val, key) in menuArr"></item-menu>
      </el-menu>
    </el-row>
  </el-aside>
</template>
