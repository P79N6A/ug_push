      ...
      <el-tabs :tab-position="tabPosition" @tab-click="handleClick">
        <el-tab-pane label="整体数据">
          <el-row class="pane-title-wrap">
            <h6 class="pane-title">
              <i class="el-icon-bell" style="margin-right: 6px;"></i>热门流-整体数据监控</h6>
          </el-row>
          <el-row type="flex">
        ...
        <el-tab-pane label="KPI指标趋势">
          <analy-page></analy-page>
        </el-tab-pane>
        <el-tab-pane label="物料日增规模">
...
import AnalyPage from "./components/AnalyPage";
...
export default {
  ...
  data() {
    return {
      ...
      isFirstArr: [false, true, true, true]
    };
  },
  computed: {
    ...mapGetters(["email", "pageAuth", "entireMonitor"])
  },
  methods: {
    init() {
      if (this.pageAuth === 1) {
    ...
    handleClick(tab, event) {
      if (tab.index !== "0" && this.isFirstArr[tab.index]) {
        this.isFirstArr[tab.index] = false;
        tab.$children[0].onSearch();
      }
    }
  },
  created() {
