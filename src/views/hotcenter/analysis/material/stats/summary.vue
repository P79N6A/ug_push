<template>
  <el-main v-if="pageAuth === 1">
    <el-row class="pane-title-wrap">
      <h6 class="pane-title">
        <i class="el-icon-printer" style="margin-right: 6px;"></i>物料构成总况</h6>
    </el-row>
        ...
        <el-row class="table-wrap" v-if="usersTable">
          <el-col>
            ...
            <el-table :data="usersTable" :default-sort="{prop: 'daily_ruku_num', order: 'descending'}" style="width: 100%" border stripe size="mini">
              ...
              <el-table-column prop="name" label="物料构成">
              </el-table-column>
              ...
              <el-table-column prop="daily_expo_weibo" label="日新增曝光物料量" sortable>
                <template slot-scope="scope">{{_formatNum(scope.row.daily_expo_weibo)}}</template>
              </el-table-column>
              <el-table-column prop="daily_expo_num" label="日新增物料曝光量" sortable>
                <template slot-scope="scope">{{_formatNum(scope.row.daily_expo_num)}}</template>
              </el-table-column>
              ...
              <el-table-column min-width="90" prop="daily_ruku_number_increase_rate" label="日新增入库物料量对比上周同期增长率" sortable>
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import("@/vendor/Export2Excel").then(excel => {
        const filename = `物料构成总况_${this.curTime}`;
        const tHeader = [
          "序号",
          "物料构成",
          "日新增入库物料量",
