    ...
    <el-main v-if="pageAuth === 1" style="padding: 20px 0;">
      <el-row type="flex" justify="center">
        <el-col :xs="24" :sm="23" :md="23" :lg="23" :xl="23">
          <el-form inline :model="formInline" label-position="right" class="search-form" size="small">
            <!-- mid尾号 -->
            <el-form-item label="mid尾号" class="active">
              <el-select v-model="formInline.last_digit" multiple placeholder="请选择mid尾号，可多选" size="small" @change="handleLastDigit()" style="width: 120px;">
            ...
            <el-form-item label="领域">
              <el-select v-model="formInline.cate" placeholder="请选择物料领域" filterable>
            ...
            <el-form-item label="物料来源" v-if="materialSourceList">
              <el-select v-model="formInline.material_source" placeholder="请选择物料来源" size="small" filterable>
                <el-option label="全部" value=""></el-option>
            ...
            <!-- 排序方式 -->
            <el-form-item label="排序方式：" style="background-color: rgb(255, 255, 255);padding: 0px 10px;border-radius: 3px">
              <el-radio-group v-model="formInline.sort_type">
                <el-radio label="time">发布时间</el-radio>
                <el-radio label="hot_num">热度</el-radio>
            ...
            <!-- 物料状态 -->
            <el-form-item label="物料状态：" style="background-color: rgb(255, 255, 255);padding: 0px 10px;border-radius: 3px">
          ...
          <!-- 审核列表 -->
          <!-- 是否全选 -->
          <div class="all-operate-box" v-if="datalist && datalist.length > 0">
          ...
          <el-checkbox-group v-model="checkedMid" @change="handleCheckedMidChange" v-if="datalist && datalist.length > 0">
            <item-list :sourcelist="materialSourceList" :key="item.mid" :callback="onSearch" :item="item" :index="(formInline.page - 1) * formInline.count + idx + 1" v-for="(item, idx) in datalist"></item-list>
          </el-checkbox-group>
...
export default {
  ...
  methods: {
    ...
    getMaterialSource() {
      const tempData = {
        url: `${api_domain}/query_source_list`,
        param: {}
      };
      const params = {
    ...
    onSearch() {
      ...
      const tempData = {
        ...
        param: {
          ...
          start_stamp: this.formInline.date && this.formInline.date[0] || "", //开始时间
          end_stamp: this.formInline.date && this.formInline.date[1] || "", //结束时间
    ...
    allDeleteEvent() {
      this.$prompt("请输入删除原因", "删除所选物料", {
        ...
        .then(() => {
          this.deleteAction(this.checkedMid.join(','), this.deleteReason);
        })
        .catch(() => {
          this.$message({
            type: "info",
